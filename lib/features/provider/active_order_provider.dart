...
class ActiveOrderProvider with ChangeNotifier {
  ...
  void addOrder(OrderGet newOrder) {
    _orders.add(newOrder);
    notifyListeners();
  }

  
  void removeOrder(OrderGet order) {
    _orders.remove(order);
    notifyListeners();
  }
  ...
  void listenToWebSocket(Stream<WebSocketMessage> webSocketStream) {
   
    _webSocketSubscription = webSocketStream.listen((message) {
      if (message.type == WebSocketMessageType.newOrder) {
        addOrder(message.data);
      } else if (message.type == WebSocketMessageType.orderExpired) {
        removeOrder(message.data); 
      }
    });
  }
